...
namespace OX.WebPort
{
    ...
    public class SwapPairMerge : ISerializable
    {
        ...
        public virtual int Size => PoolAddress.Size + SwapPairReply.Size + TxId.Size + TargetAssetState.Size + sizeof(uint) + sizeof(uint) + (IDO.IsNotNull() ? IDO.Size : 0);
        public void Serialize(BinaryWriter writer)
        {
            writer.Write(PoolAddress);
        ...
        public void Deserialize(BinaryReader reader)
        {
            ...
            if (c > 0)
            {
                IDO = reader.ReadSerializable<SwapPairIDO>();
            }
        }
    }
