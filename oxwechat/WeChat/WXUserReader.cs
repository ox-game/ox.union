...
//                    continue;
        ...
        public List<WXMsg>? GetWXMsgs(string uid, MarkTerm term)
        {
            List<WXMsg> tmp = new List<WXMsg>();
            for (int i = 0; i <= 99; i++)
            {
                ...
                wXMsgs = con.Query<WXMsg>(query, uid, term.ToDateTime().ToString("yyyy-MM-dd"));
                //}
        ...
        public void Close()
        {
           foreach(var conn in  DBInfo.Values)
            {
                conn.Close();
            }
        }
