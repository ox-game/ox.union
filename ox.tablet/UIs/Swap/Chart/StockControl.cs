...
namespace OX.Tablet.Chart
{
    public partial class StockControl : ContainerControl
    {
        bool IsHour = false;
        public StockControl(SwapPairMerge swapPairMerge)
        {
        ...
        KeyValuePair<SwapPairExchangeKey, SwapVolumeMerge> LastPair;
        ...
        {
            //new MySwapRecords(this.Operater, this.SwapPairMerge, this.LoadData).ShowDialog();
        }
        ...
        public void Init(IEnumerable<KeyValuePair<SwapPairExchangeKey, SwapVolumeMerge>> loaddata)
        {
            ...
            if (this.LoadData.IsNotNullAndEmpty())
            {
                ...
                foreach (var p in this.LoadData.OrderByDescending(m => (long)m.Value.Volume.BlockIndex * 1000 + (long)m.Value.Volume.TxN))
                {
                    ...
                    if (p.Value.Volume.TargetAssetVolume > Fixed8.Zero)
                    {
                        ...
                        assetName = this.SwapPairMerge.TargetAssetState.GetName();
                        ok = true;
                    }
        ...
        public void DrawScale(Graphics g, out int klbottom, out decimal? mp, out decimal? lp)
        {
            ...
            var targetAssetName = this.SwapPairMerge.TargetAssetState.GetName();
