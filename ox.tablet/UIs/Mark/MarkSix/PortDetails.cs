...
namespace OX.Tablet
{
    ...
    public partial class PortDetails : UserControl, IBlockchainHandler
    {
        ...
        public PortDetails(MixMarkMember member)
        {
            this.Member = member;
            InitializeComponent();
        ...
        void ReloadRecords()
        {
            ...
            {
                Fixed8 PortTotalBetAmount = Fixed8.Zero;
                Fixed8 TermTotalBetAmount = Fixed8.Zero;
                foreach (var r in bmsIndex.MarkPortPlayerTermRecords(this.CurrentSelectedTerm, SecureHelper.MasterAccount.ScriptHash))
                {
                    if (bmsIndex.MarkMembers.TryGetValue(r.Key.Player, out var member))
                    {
                        var control = new PortOrder(member, new MarkPortPlayerTermRecord { MarkPortPlayerTermKey = r.Key, MarkPortPlayerTermValue = r.Value });
                        pn_orders.Add(control);
                        PortTotalBetAmount += r.Value.TotalBetAmount;
                    }
                }
                foreach (var r in bmsIndex.MarTermAllRecords(this.CurrentSelectedTerm))
                {
                    TermTotalBetAmount += r.Value.TotalBetAmount;
                }
                this.L1.Text = UIHelper.LocalString($"盘口总投注额:{PortTotalBetAmount.ToString("f0")},全链总投注额:{TermTotalBetAmount.ToString("f0")}", $"Port Total Bet:{PortTotalBetAmount.ToString("f0")},Blockchain Total Bet:{TermTotalBetAmount.ToString("f0")}");
            }
        }
