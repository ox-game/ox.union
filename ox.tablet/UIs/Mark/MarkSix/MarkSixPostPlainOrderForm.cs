...
{
    public partial class MarkSixPostPlainOrderForm : UIForm
    {
        TreeNode[] Nodes;
        ...
        public MarkSixPostPlainOrderForm(TreeNode[] nodes, MarkSixPlazaForm parentForm)
        {
            this.Nodes = nodes;
        ...
        private void DoPost()
        {
            ...
            var OrderMerge = BitSixAgentHelper.CombinePlainOrders(SecureHelper.MasterAccount.ScriptHash, term, list.ToArray(), NodeConfig.Instance.AutoCutMarkBet);
            ...
            agentIndex.Do(batch =>
            {
                ...
                {
                    var orders = OrderMerge.Order.PlainShard();
                    if (orders.IsNotNull())
                    {
                        ...
                        {
                            var key = new MarkPlainBetOrderKey { Player = order.Player, ChannelRound = order.ChannelRound, Nonce = nonce, Term = term, OrderHash = order.Hash };
                            var value = new MarkPlainBetOrderValue { Order = order, State = MarkPlainOrderStatus.Inited };
                            agentIndex.SavePlainOutboundOrder(batch, new MarkPlainTermBetOrder { Key = key, Value = value });
