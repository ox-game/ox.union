...
{
    public partial class TMColorBetForm : BaseBetForm
    {
        public virtual uint LeastCodeNumber { get; } = 1;
        public TMColorBetForm()
        {
            InitializeComponent();
        }
        ...
        {
            var methodSetting = MarkSixBetMethod.TM_Color.GetMethodSetting();
        ...
        public override BitMarkSixBetTarget[] GetBetTargets()
        {
            ...
            foreach (PointSwitch ps in this.GetPointControls())
            {
                if (ps.Checked)
                {
                    ...
                    {
                        Method = (byte)MarkSixBetMethod.TM_Color,
